...
export const trips: Trip[] = [
  ...
  // ═══════════════════════════════════════════════════════
  {
    ...
    flights: [
      ...
      { segment: 5, date: '2026-08-12', route: 'FRA → SEA', time: '14:15 → 15:40', airline: 'Condor (DE 2032)', aircraft: 'A330-900neo', cabin: 'Business', confirmation: 'A9XYQ9 / JUUZBO', status: 'booked', notes: '10h 25m nonstop. Condor conf A9XYQ9, JetBlue conf JUUZBO (Condor–JetBlue partnership). Arrive Seattle 15:40.' },
  ...
  // ═══════════════════════════════════════════════════════
  {
    ...
    flights: [
      ...
      { segment: 3, date: 'TBD', route: 'LHR → SGN → ICN', airline: 'Vietnam Airlines', cabin: 'Business', status: 'booked', notes: 'London to Seoul via Saigon (Ho Chi Minh City/SGN). 80K points + $75. PNR TBD.' },
      { segment: 4, date: '2026-10-25', route: 'ICN → SEA', time: '21:00 → 14:55', airline: 'Asiana Airlines (OZ272)', cabin: 'Business (I)', confirmation: 'TBD', status: 'booked', notes: 'ICN Terminal 2. 9h 55m. Seat 77L. 75K Aeroplan + CA$120.50. Booked via Aeroplan.' },
    ],
    hotels: [
    ...
    actionItems: [
      { text: 'CANCEL original SEA→ICN booking — replaced by SEA→FRA→LHR→SGN→ICN routing', urgent: true },
      { text: 'Book FRA→LHR connecting flight (Oct 12, after 12:45 PM Condor arrival)', urgent: true },
      { text: 'Confirm Vietnam Airlines LHR→SGN→ICN PNR and travel dates', urgent: true },
    ...
    awardTips: [
      { program: 'JetBlue TrueBlue', route: 'SEA → FRA', cost: '75K + $22.50', note: 'Condor DE 2033 Oct 11. Booked.' },
      { program: 'Vietnam Airlines (SkyTeam)', route: 'LHR → SGN → ICN', cost: '80K + $75', note: 'Booked via SkyTeam partner. Via Saigon.' },
      { program: 'Aeroplan', route: 'ICN → SEA', cost: '75K + CA$120.50', note: 'Asiana OZ272 Oct 25. Booked.' },
    ],
    itinerary: [
      { date: 'October 11', label: 'Depart Seattle → Frankfurt', description: 'Condor DE 2033 SEA 5:40 PM → FRA Oct 12 12:45 PM. Terminal 1. 75K JetBlue + $22.50.', type: 'travel' },
      { date: 'October 12', label: 'Frankfurt → London (Work Trip)', description: 'Arrive FRA 12:45 PM. Connect to LHR (TBD — not yet booked). London work trip begins.', type: 'travel' },
      { date: 'TBD', label: 'London → Seoul via Saigon', description: 'Vietnam Airlines LHR → SGN → ICN. 80K points + $75. Dates TBD pending London work schedule.', type: 'travel' },
      { date: 'TBD', label: 'Seoul — Arrive & Check In', description: 'Land at Incheon Terminal 2. AREX to Seoul Station. Check into Hannam-dong hotel. Rest. Evening walk through Itaewon.', type: 'travel' },
      { date: 'TBD', label: 'Hannam-dong & Ikseon-dong', description: 'Morning coffee and galleries in Hannam-dong. Afternoon hanok cafés in Ikseon-dong. Dinner at Gwangjang Market.', type: 'activity' },
      { date: 'TBD', label: 'K-Beauty & Garosugil', description: 'Morning at Garosugil — K-beauty flagships. Lunch at a design café. Afternoon Myeongdong shopping. Evening wine bar.', type: 'activity' },
      { date: 'TBD', label: 'Autumn Foliage', description: 'Namsan hike for foliage views. Han River golden hour. Lunch in Yeonnam-dong. Afternoon at Leeum Museum.', type: 'activity' },
      { date: 'October 25', label: 'Fly Home', description: 'Check out hotel. AREX to Incheon T2. Asiana OZ272 ICN 21:00 → SEA 14:55 (9h 55m, Business I, seat 77L). 75K Aeroplan + CA$120.50.', type: 'travel' },
    ],
    costs: [
      // ─── Flights ───
      { category: 'flight', description: 'SEA → ICN original booking — CANCEL', note: '⚠️ Cancel this. Replaced by new routing. Pull PNR from janelle.tam@gmail.com.', canceled: true },
      { category: 'flight', description: 'SEA → FRA on Condor DE 2033 (Oct 11)', points: 75000, program: 'JetBlue TrueBlue', cashUsd: 22.50, note: '5:40 PM → 12:45 PM Oct 12. Terminal 1. Business.' },
      { category: 'flight', description: 'FRA → LHR (Oct 12)', note: 'Not yet booked. After 12:45 PM Condor arrival.' },
      { category: 'flight', description: 'LHR → SGN → ICN on Vietnam Airlines', points: 80000, cashUsd: 75, note: 'Via Saigon. Business. PNR TBD.' },
      { category: 'flight', description: 'ICN → SEA on Asiana OZ272 (Oct 25)', points: 75000, program: 'Aeroplan', cashUsd: 88, note: 'ICN T2 21:00 → SEA 14:55. Business (I). Seat 77L. CA$120.50 (~$88 USD).' },
      // ─── Lodging ───
  ...
  // ═══════════════════════════════════════════════════════
  {
    ...
    flights: [
      ...
      { segment: 2, date: '2027-02-06', route: 'NRT → CTS', airline: 'JAL or ANA', status: 'needs_action', notes: 'Need to book. ~90 min. Then shuttle from New Chitose to Niseko (~2.5 hrs). Book soon — Feb peak.' },
      { segment: 3, date: '2027-02-11', route: 'CTS → KIX', airline: 'JAL or ANA', status: 'needs_action', notes: 'Need to book. ~2 hrs. Then train or taxi to Kyoto (75 min from KIX).' },
      { segment: 4, date: '2027-02-13', route: 'KIX → TPE', airline: 'EVA Air or JAL', status: 'needs_action', notes: 'Need to book. ~3 hrs. EVA likely (already on EVA return). Connects to Taipei days before BR24 return Feb 16.' },
      { segment: 5, date: '2027-02-16', route: 'TPE → SEA', time: '11:00pm → 5:30pm', airline: 'EVA Air (BR24)', cabin: 'Business', confirmation: 'FK5O97', status: 'booked', notes: 'Return via Taipei. EVA Royal Laurel Class. 75K EVA miles + NT$4,000 per person (~$130 USD/pp). Both Terry & Janelle.' },
    ],
    hotels: [
      ...
      { property: 'Park Hyatt Kyoto', location: 'Higashiyama, Kyoto', checkIn: '2027-02-11', checkOut: '2027-02-13', program: 'Hyatt', status: 'booked', notes: 'Conf: 35675427. 2 nights. Ninenzaka location — steps from Kiyomizu-dera. Booking dates need adjustment at Hyatt (was Feb 10–15).' },
      { property: 'Park Hyatt Kyoto — CANCEL', location: 'Higashiyama, Kyoto', checkIn: '2027-02-10', checkOut: '2027-02-15', program: 'Hyatt', status: 'pending', notes: '⚠️ CANCEL — Conf: 47489247. Room 2 not needed (only 2 travelers, 1 room).' },
      { property: 'Park Hyatt Niseko Hanazono — CANCEL', location: 'Niseko, Hokkaido', checkIn: '2027-02-13', checkOut: '2027-02-18', program: 'Hyatt', status: 'pending', notes: '⚠️ CANCEL — Conf: 2552461. Not returning to Niseko — going to Taipei Feb 13 instead.' },
      { property: 'Park Hyatt Tokyo — CANCEL', location: 'Shinjuku, Tokyo', checkIn: '2027-02-14', checkOut: '2027-02-18', program: 'Hyatt', status: 'pending', notes: '⚠️ CANCEL — Conf: 50440509. Trip ends in Taipei, not Tokyo.' },
      { property: 'TBD — Taipei hotel', location: 'Taipei, Taiwan', checkIn: '2027-02-13', checkOut: '2027-02-16', status: 'not_booked', notes: '3 nights. Arrives via KIX→TPE on Feb 13. Departs TPE→SEA (EVA BR24 FK5O97) on Feb 16. Need to book.' },
    ],
    actionItems: [
      { text: 'Book NRT → CTS internal flight (Feb 6) — JAL or ANA, ~90 min', urgent: true },
      { text: 'Book Niseko shuttle from New Chitose airport (CTS) to Niseko — ~2.5 hrs', urgent: true },
      { text: 'Book CTS → KIX internal flight (Feb 11) — JAL or ANA, ~2 hrs', urgent: true },
      { text: 'Book KIX → TPE flight (Feb 13) — EVA Air likely; connects to BR24 return Feb 16', urgent: true },
      { text: 'Book Taipei hotel (Feb 13–16, 3 nights)', urgent: true },
      { text: 'CANCEL Park Hyatt Kyoto Room 2 (conf: 47489247)', urgent: true },
      { text: 'CANCEL Park Hyatt Niseko 2nd stay (conf: 2552461)', urgent: true },
      { text: 'CANCEL Park Hyatt Tokyo return stay (conf: 50440509)', urgent: true },
      { text: 'Adjust Park Hyatt Kyoto conf 35675427 to Feb 11–13 (was Feb 10–15 — dates shifted)', urgent: true },
    ...
    awardTips: [
      ...
      { program: 'EVA Infinity MileageLands', route: 'TPE–SEA', cost: '75K + NT$4,000 per pax = 150K + NT$8,000 total (~$265 USD)', note: 'Royal Laurel Class return. Both Terry & Janelle on FK5O97.' },
    ...
    itinerary: [
      ...
      { date: 'Feb 6', label: 'Tokyo → Niseko', description: 'Check out Park Hyatt Tokyo. Morning flight NRT → CTS (New Chitose, ~90 min, needs booking). Shuttle from CTS to Niseko (~2.5 hrs through snowy Hokkaido countryside). Check in to Park Hyatt Niseko Hanazono (conf: 19374554). Afternoon orientation ski run. Evening: onsen at the hotel, ramen in Kutchan.', type: 'travel' },
      { date: 'Feb 7–10', label: 'Niseko Powder Days', description: '4 full days of Japow skiing. Hanazono Bowl, Grand Hirafu, Niseko Village terrain. Morning first tracks, afternoon tree runs. Night skiing sessions. Daily onsen recovery. Ramen, izakaya, and hot pot dinners.', type: 'activity' },
      { date: 'Feb 11', label: 'Niseko → Kyoto', description: 'Check out Park Hyatt Niseko (conf: 19374554). Fly CTS → KIX (~2 hrs, needs booking). Train or taxi to Kyoto (75 min from KIX). Check in to Park Hyatt Kyoto (conf: 35675427). Evening walk through Higashiyama — stone lanes, tea houses, temple silhouettes at dusk. Kaiseki dinner.', type: 'travel' },
      { date: 'Feb 12', label: 'Kyoto Cultural Day', description: 'Morning: Fushimi Inari before 8am (thousand torii gates, no crowds). Kinkaku-ji (Golden Pavilion). Arashiyama bamboo grove. Afternoon: tea ceremony experience, Nishiki Market food crawl. Kaiseki at Kikunoi or Gion Sasaki. Gion geisha district evening walk.', type: 'activity' },
      { date: 'Feb 13', label: 'Kyoto → Taipei', description: 'Check out Park Hyatt Kyoto (conf: 35675427). Transfer to KIX and fly KIX → TPE (~3 hrs, needs booking). Check into Taipei hotel (needs booking). First evening in Taipei — Shilin Night Market or Ximending.', type: 'travel' },
      { date: 'Feb 14–15', label: 'Taipei Days', description: 'Explore Taipei. Jiufen old town day trip (the Spirited Away village, coastal views). Taipei 101 + Xinyi shopping. Din Tai Fung for soup dumplings. Da\'an Park neighborhood. Elephant Mountain sunset hike.', type: 'activity' },
      { date: 'Feb 16', label: 'Fly Home', description: 'Final morning in Taipei. TPE 11:00pm → SEA 5:30pm on EVA Air BR24 Royal Laurel Class (conf: FK5O97). Home same day — date line gain. 12-day trip complete.', type: 'travel' },
    ],
    costs: [
  ...
  // ═══════════════════════════════════════════════════════
  {
    ...
    flights: [
      ...
      { segment: 2, date: '2027-05-23', route: 'AMM → DFW', time: '10:45 AM → 5:05 PM', airline: 'Royal Jordanian (RJ 291)', cabin: 'Business', confirmation: 'KKXCNJ / BJGT5Y', status: 'booked', notes: '14h 20m nonstop. AA conf KKXCNJ, RJ conf BJGT5Y. 70K AAdvantage + $136/pp × 2 pax = 140K miles + $272.' },
    ],
    hotels: [
    ...
    actionItems: [
      { text: 'Book hotels in Jordan (Amman, Petra, Wadi Rum)', urgent: true },
    ...
    costs: [
      ...
      { category: 'flight', description: 'AMM → DFW on Royal Jordanian RJ 291 (May 23, 2 pax)', pnr: 'BJGT5Y', program: 'AAdvantage', points: 140000, cashUsd: 272, note: '70K AA miles + $136/pp × 2 = 140K miles + $272 total.' },
